...
#        plt.plot(iters, self.losses[loss_type], 'g', label='train loss')
        plt.plot(iters, list(np.array(self.losses[loss_type])/10), 'g', label='train loss')
        if loss_type == 'epoch':
            # val_acc
            plt.plot(iters, self.val_acc[loss_type], 'b', label='val acc')
            # val_loss
#            plt.plot(iters, self.val_loss[loss_type], 'k', label='val loss')
            plt.plot(iters,  list(np.array(self.val_loss[loss_type])/10), 'k', label='val loss')
        plt.grid(True)
        plt.xlabel(loss_type)
        plt.ylabel('acc-loss')
...
# 编译模型
model.compile(optimizer='adam', loss='categorical_crossentropy',  metrics = ['accuracy'])  #rmsprop   [focal_loss(gamma=2)]
